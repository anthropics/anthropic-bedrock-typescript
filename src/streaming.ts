...
import { AnthropicBedrockError } from './error';

import { safeJSON, createResponseHeaders } from '@anthropic-ai/bedrock-sdk/core';
import { APIError } from '@anthropic-ai/bedrock-sdk/error';
...
export class Stream<Item> implements AsyncIterable<Item> {
  ...
  static fromSSEResponse<Item>(response: Response, controller: AbortController) {
    ...
    async function* iterMessages(): AsyncGenerator<ServerSentEvent, void, unknown> {
      if (!response.body) {
        controller.abort();
        throw new AnthropicBedrockError(`Attempted to iterate over a response with no body`);
      }
